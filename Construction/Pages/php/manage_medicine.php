...
function showEditOptionsRow($seq_no, $row) {
  ...
  <tr>
    <td><?php echo $seq_no; ?></td>
    <td>
      <input type="text" class="form-control" value="<?php echo $row['NAME']; ?>" placeholder="Material Name" id="medicine_name" onblur="notNull(this.value, 'medicine_name_error');">
    ...
    <td>
      <input type="text" class="form-control" value="<?php echo $row['REMARKS']; ?>" placeholder="Remarks" id="generic_name" onblur="notNull(this.value, 'generic_name_error');">
...
function updateMedicine($id, $name, $packing, $generic_name, $suppliers_name) {
  require "db_connection.php";
  $query = "UPDATE materials SET NAME = '$name', PACKING = '$packing', REMARKS = '$generic_name', SUPPLIER_NAME = '$suppliers_name' WHERE ID = $id";
  $result = mysqli_query($con, $query);
  if(!empty($result))
    showMedicines(0);
}
...
function searchMedicine($text, $tag) {
  ...
  if($con) {
    $seq_no = 0;
    $query = "SELECT * FROM materials WHERE UPPER($column) LIKE '%$text%'";
    $result = mysqli_query($con, $query);
    while($row = mysqli_fetch_array($result)) {
      $seq_no++;
      showMedicineRow($seq_no, $row);
