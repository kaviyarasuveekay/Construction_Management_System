<?php
  ...
  function showInvoices() {
    ...
    if($con) {
      $seq_no = 0;
      $query = "SELECT * FROM invoices INNER JOIN workers ON invoices.WORKERS_ID = WORKERS.ID";
      $result = mysqli_query($con, $query);
  ...
  function searchInvoice($text, $column) {
    ...
    if($con) {
      ...
      if($column == 'INVOICE_ID')
        $query = "SELECT * FROM invoices INNER JOIN workers ON invoices.WORKERS_ID = WORKERS.ID WHERE CAST(invoices.$column AS VARCHAR(9)) LIKE '%$text%'";
      else if($column == "INVOICE_DATE")
        $query = "SELECT * FROM invoices INNER JOIN workers ON invoices.WORKERS_ID = WORKERS.ID WHERE invoices.$column = '$text'";
      else
        $query = "SELECT * FROM invoices INNER JOIN workers ON invoices.WORKERS_ID = WORKERS.ID WHERE UPPER(workers.$column) LIKE '%$text%'";
  ...
  function printInvoice($invoice_number) {
    require "db_connection.php";
    if($con) {
      $query = "SELECT * FROM sallary INNER JOIN workers ON sallary.CUSTOMER_ID = WORKERS.ID WHERE INVOICE_NUMBER = $invoice_number";
      $result = mysqli_query($con, $query);
      $row = mysqli_fetch_array($result);
      $customer_name = $row['NAME'];
      $address = $row['ADDRESS'];
      $contact_number = $row['CONTACT_NUMBER'];
      $doctor_name = $row['CONTRACTOR_NAME'];
      $doctor_address = $row['REMARKS'];
    ...
    <div class="row">
      ...
      <div class="col-md-10 h3" style="color: #ff5252;">Workers Invoice<span class="float-right">Invoice Number : <?php echo $invoice_number; ?></span></div>
    </div>
    ...
    <div class="row">
      <div class="col-md-1"></div>
      <div class="col-md-4">
        <span class="h4">Worker Details : </span><br><br>
        ...
        <span class="font-weight-bold">Contractor's Name : </span><?php echo $doctor_name; ?><br>
      </div>
      ...
      <div class="col-md-4">
        <span class="h4">Site Details : </span><br><br>
        ...
        <table class="table table-bordered table-striped table-hover" id="purchase_report_div">
          ...
          <tbody>
            <?php
              $seq_no = 0;
              $total = 0;
              $query = "SELECT * FROM sallary WHERE INVOICE_NUMBER = $invoice_number";
              $result = mysqli_query($con, $query);
              while($row = mysqli_fetch_array($result)) {
                $seq_no++;
                ?>
