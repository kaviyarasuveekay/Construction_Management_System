<?php
  ...
  function isNewMedicine($name, $packing) {
    require "db_connection.php";
    if($con) {
      $query = "SELECT * FROM materials WHERE UPPER(NAME) = '$name' AND UPPER(PACKING) = '$packing'";
      $result = mysqli_query($con, $query);
      $row = mysqli_fetch_array($result);
      echo ($row) ? "false" : "true";
    }
  ...
  function addStock() {
    ...
    if($con) {
      $query = "SELECT * FROM materials_stock WHERE UPPER(NAME) = '".strtoupper($name)."' AND UPPER(BATCH_ID) = '$batch_id'";
      $result = mysqli_query($con, $query);
      $row = mysqli_fetch_array($result);
      if($row) {
        $new_quantity = $row['QUANTITY'] + $quantity;
        $query = "UPDATE materials_stock SET QUANTITY = $new_quantity WHERE UPPER(NAME) = '".strtoupper($name)."' AND UPPER(BATCH_ID) = '$batch_id'";
        $result = mysqli_query($con, $query);
      }
      else {
        $query = "INSERT INTO materials_stock (NAME, BATCH_ID, EXPIRY_DATE, QUANTITY, MRP, RATE) VALUES('$name', '$batch_id', '$expiry_date', $quantity, $mrp, $rate)";
        $result = mysqli_query($con, $query);
      }
    }
  }
  ...
  function createMedicineInfoRow() {
      ...
      <div class="row col col-md-12">
        <div class="col col-md-2">
          <input type="text" class="form-control" placeholder="Material Name" name="medicine_name">
      ...
      <div class="row col col-md-8">
        <div class="col col-md-4"><label for="generic_name" class="font-weight-bold">&nbsp;Remarks : </label></div>
        <div class="col col-md-8">
          <input type="text" class="form-control" placeholder="Remarks" name="generic_name">
