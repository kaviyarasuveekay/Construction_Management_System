<?php
  ...
  function showMedicinesStock($id) {
    ...
    if($con) {
      $seq_no = 0;
      $query = "SELECT * FROM materials INNER JOIN materials_stock ON materials.NAME = materials_stock.NAME";
      $result = mysqli_query($con, $query);
...
function updateMedicineStock($id, $batch_id, $expiry_date, $quantity, $mrp, $rate) {
  require "db_connection.php";
  $query = "UPDATE materials_stock SET BATCH_ID = '$batch_id', EXPIRY_DATE = '$expiry_date', QUANTITY = $quantity, MRP = $mrp, RATE = $rate WHERE ID = $id";
  $result = mysqli_query($con, $query);
  if(!empty($result))
    showMedicinesStock("0");
}
...
function searchMedicineStock($text, $column) {
  ...
  if($con) {
    ...
    if($column == "EXPIRY_DATE")
      ...
      $query = "SELECT * FROM materials INNER JOIN materials_stock ON materials.NAME = materials_stock.NAME WHERE materials_stock.$column <= 5";
    else
      $query = "SELECT * FROM materials INNER JOIN materials_stock ON materials.NAME = materials_stock.NAME WHERE UPPER(materials.$column) LIKE '%$text%'";
