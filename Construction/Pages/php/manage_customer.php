...
function showEditOptionsRow($seq_no, $row) {
  ...
  <tr>
    ...
    <td>
      <input type="text" class="form-control" value="<?php echo $row['CONTRACTOR_NAME']; ?>" placeholder="Contractor's Name" id="customer_doctors_name" onkeyup="validateName(this.value, 'doctor_name_error');">
      <code class="text-danger small font-weight-bold float-right" id="doctor_name_error" style="display: none;"></code>
    </td>
    <td>
      <textarea class="form-control" placeholder="Remarks" id="customer_doctors_address" onblur="validateAddress(this.value, 'doctor_address_error');"><?php echo $row['REMARKS']; ?></textarea>
...
function updateCustomer($id, $name, $contact_number, $address, $doctor_name, $doctor_address) {
  require "db_connection.php";
  $query = "UPDATE workers SET NAME = '$name', CONTACT_NUMBER = '$contact_number', ADDRESS = '$address', CONTRACTOR_NAME = '$doctor_name', REMARKS = '$doctor_address' WHERE ID = $id";
  $result = mysqli_query($con, $query);
  if(!empty($result))
    showCustomers(0);
}
